...
### 火力单元模型（核心粒度）
- **点 ≠ 士兵**：一个点代表一个火力单元（`soldiersPerDot(wing)`：全翼统一 22 人）。每排容量 `rowCapacity(wing, rowWidth)`：每排长度 = 战场宽度（人），点数 = ceil(rowWidth / 每点人数)。改布点/槽位时必须保持"点数 = ceil(兵力 / 每点人数)"这一换算一致。
- **前排固定三排**：`formationRows` 中前排只要还有单元就铺成 3 排（`distributeUnits` 均匀分布，默认宽度下如中军 30/30/30、两翼 8/8/8）；中排/后排按「前排单行最大点数」同宽切行（也可多排），没有前排时退回战场宽度容量。
- **三段击（射击节奏，核心）**：一点 = 一个排；前三排的同一列（row 0/1/2 × 同 col）= 一个连。连内三排依次开火——第 `planTick % RANK_CYCLE` 排当回合开火（`RANK_CYCLE = 3`，layout.ts 导出），每排 3 回合射一次，每回合恰好一整排持续射击（溃退/撤退中的翼不射）。单发命中概率按 `RANK_CYCLE` 补偿（聚合杀伤率 = coeff × damageScale，与原 14 回合轮射设计一致）；**不要再乘单元规模**（单元规模在"射击次数 × 每点人数"中抵消，重复计入会让战斗快几十倍）。注意三段击射击更"实弹"（每回合约 1/3 前排开火，旧轮射仅 1/14），默认配置下战斗约缩短 20%（约 750~800 回合，旧模型约 900~1050）。
...
- 默认配置（index.html）：双方各 10000 人、翼部署 17/66/17、梯队 30/30/40、宽度 4500、火炮 10/40/10、骑兵 4/12/4、伤害 0.014（三段击下随机默认约 750~800 回合，火炮占比约 7%）。
...
### 关键不变量（模拟正确性）
- **仅前排开火**：只有前排火力单元参与攻击分配，且按三段击节奏（每回合整排轮转）开火；溃退/撤退中的翼不攻击。
...
- 击杀红叉按 `KILL_MARK_TICKS = 8` 回合保留（暂停也显示），与墙钟无关；士兵点与被摧毁的火炮图标都会标红叉；击杀实线按 `KILL_LINE_TURNS = 4` 回合保留（单回合击杀稀疏，保留窗口让击杀线可见），墙钟窗口是 1 秒。
