...
- **前排固定三排**：`formationRows` 中前排只要还有单元就铺成 3 排（`distributeUnits` 均匀分布，默认宽度下如中军 30/29/29、两翼 10/10/10）；中排/后排按容量逐排。
...
- 骑兵（离散状态机）：按翼配置（类似火炮），部署在步兵与火炮之间；每个单元独立在「准备（蓄力 30 回合）→ 冲锋 → 准备」间循环。准备状态不受任何伤害（当前所有火力都只指向士兵点与火炮，天然不会打到骑兵）；冲锋时做一次对决（胜率 0.5 × 随机性噪声）：成功必杀目标火力单元、自身无损，失败则自身阵亡一个单元、目标无损失。骑兵目标遵守当面翼约束（`fireTargets`），溃退/撤退中的翼不冲锋；对决与蓄力都走确定性种子。
